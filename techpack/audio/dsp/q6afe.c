...
int afe_dsm_ramp_dn_cfg(uint8_t *payload, uint32_t delay_in_ms)
{
	...
	*(params + 5) = 500; /* 5s mute time will make sure silence output */
...
int afe_dsm_pre_calib(uint8_t *payload)
{
	...
	*(params + 3) = 4; /* mode 0: disable, 1: enable, 2: bypass and
			      pilot tone, 4: pilot tone only */

	afe_dsm_rx_set_params(payload, 4 * sizeof(uint32_t));
	/* make the stable iv data */
...
int afe_dsm_post_calib(uint8_t *payload)
{
	...
	*(params + 1) = 1; /* count */
	*(params + 2) = 0x03000001; /* enable flag */
	*(params + 3) = 1; /* mode 0: disable, 1: enable, 2: bypass and
			      pilot tone, 4: pilot tone only */
